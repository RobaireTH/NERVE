...
//
// State machine:
//   Open (0) → Reserved (1): poster hands cell to worker
//   Reserved (1) → Claimed (2): poster commits worker to the job
//   Claimed (2) → Completed (3): worker completed; cell may be destroyed to release reward
//   Any → Expired (4): poster cancels; cell destroyed and capacity reclaimed
//
// Invariants enforced by this type script:
//   - Status can only advance or jump to Expired (4).
//   - poster_lock_args, reward, capability_hash, and ttl are immutable after creation.
//   - Cell destruction (settlement) is always allowed — the lock script controls who can do it.
...
fn validate_creation() -> Result<(), i8> {
	...
	if data[1] != 0 { return Err(ERR_INVALID_STATUS); }
...
fn validate_transition() -> Result<(), i8> {
	...
	// If the cell doesn't appear in outputs, it's being destroyed (settlement/cancellation).
	// The lock script ensures only the authorized party can do this.
	...
	if new_status != STATUS_EXPIRED && new_status <= old_status {
	...
	if old[42..50] != new[42..50] { return Err(ERR_IMMUTABLE_FIELD_CHANGED); }
	if old[50..58] != new[50..58] { return Err(ERR_IMMUTABLE_FIELD_CHANGED); }
	if old[58..90] != new[58..90] { return Err(ERR_IMMUTABLE_FIELD_CHANGED); }
